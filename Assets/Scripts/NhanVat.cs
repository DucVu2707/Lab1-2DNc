...
public class NhanVat : MonoBehaviour
{
    ...
    void updateMet(){
        // float playerMet = transform.position.x;
        // float meters = Mathf.Abs(playerMet);
        // metText.text = meters.ToString("FO") + "m";

         // Lấy vị trí hiện tại của người chơi theo trục X
        float playerX = transform.position.x;

        // Tính số mét dựa trên vị trí hiện tại của người chơi
        float meters = Mathf.Abs(playerX);

        // Hiển thị số mét trên Text
        metText.text = meters.ToString("F0") + "m";
    }
    void diChuyenPhai()
    {
